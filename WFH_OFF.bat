...
:L1_CHOICE_2
	msg /time:10 "%username%" "時間到，[程式自動結束]。並執行後續作業!!!"
	::: 時間到，程式自動結束
	GOTO L1_GO_KILL
	:L1_END_CHOICE_2
	GOTO L1_EXIT
...
:L1_CHOICE_1
	msg /time:10 "%username%" "您剛剛選擇 [ 取消(不處理) ]。故未執行任何後續作業 !!!"
	GOTO L1_EXIT
	
:L1_CHOICE_0
	msg /time:10 "%username%" "您選擇 [確認並執行]。程式將執行後續作業!!!"
	GOTO L1_GO_KILL
	:L1_END_CHOICE_0
	GOTO L1_EXIT
...
<input type="button" value="貪睡(10分)" onclick="mySLEEP()"
style="width:200px;height:46px;border:5px #007cd1 double;font-size:30px;color:blue;">
...
<p> [ 如未按下 "貪睡"，來休眠接收訊息， <span id="timer"></span>秒，後自動離開並自動執行後續程式!!!]  </p>
</center>
