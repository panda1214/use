...
echo Set objShell = CreateObject("WScript.Shell") > %tmp%\temp2.vbs
::echo X=MsgBox(%tmp%\bcdedit.log,vbOKOnly+vbInformation,"bcdedit.log"^) >> D:\temp_msg2.vbs
echo X=MsgBox(D:\bcdedit.txt,vbOKOnly+vbInformation,"bcdedit.log"^) >> D:\temp_msg2.vbs
>>D:\temp_msg2.vbs echo wscript.quit X

echo read_txt.exe %tmp%\bcdedit.log
::call read_txt.exe %tmp%\bcdedit.log
