@echo off
::REM ****************************************************
::2021-0125 新增明日為周六(一般)例假日時,改再檢查周1
::REM
::判別明日是否上班  (1)補上班工作日 (2)周六或周日 (3)例假日
...
::REM (1) 檢查明日是否存在   workday.dat 為補上班工作日
::REM (2) 比對明日日期有無在 holiday.dat 如有代表明日為設定例假日
::REM (3) 檢查明日是否為 周六或周日 ,    如有代表明日為勞工(一般)例假日
::REM ****************************************************
::REM workday.dat
::REM 20210220
::REM 20210911
...
set M1_TODAY=%x%%y%%z%
REM M1_TODAY=YYYYMMDD  	 明日日期
set M1_WORK_YN=1
::REM M1_WORK_YN=1  	     預設明日日期上班工作日 (1->  true(A) fail(go step2)   2->true(Y) fail(go step3) 		3->true(N) fail (S)
set M1_WEEK_D=0
REM M1_WEEK_D=0 	     預設明日周 0
...
SET M1_MSG_CHT_STR="[檢查中]_明日日期(01)%M1_TAG_WRK_FIL1% (2)系統周幾 (3)%M1_TAG_HOL_FIL1%-[FM-%M1_SENDIP%]"
REM !!!####20190121######
	SET M1_RUNDRIVE=D:
...
echo. **************************************************	>> %M1_LOG_HEAD%
echo. 程式名稱 Find_TM_Holiday.BAT							>> %M1_LOG_HEAD%
...
REM M1_TODAY=YYYYMMDD  	 明日日期
set M1_WORK_YN=00
REM M1_WORK_YN=10  SUN FUNC true 明日日期上班工作日 GOTO L1_CHECK_STEP_END
REM M1_WORK_YN=11  SUN FUNC fail 明日日期上班工作日 goto step 2
...
set M1_WORK_YN=11
SET M1_MSG_CHT_STR="[檢查中]_STEP02-%M1_TAG_HOL_FIL1% 找出 %M1_TODAY% 有無設定明日補休假日-[FM-%M1_SENDIP%]"	
echo END.....Process...Step 1 :未找到 %M1_TODAY% 存在 %M1_TAG_WRK_FIL1% 為補上班工作日,進入Step 2檢查.		>> %M1_LOG%
REM M1_WORK_YN=2  SUN FUNC fail 明日日期上班工作日 goto step 2
...
:L1_CHECK_STEP02
REM *************************************************************************************************
REM : 找出    周6,日 有無在 holiday.dat 如有代表明日為非周6周日之補休假日
REM :: SUB FUNC L1_CHECK_STEP02
SET M1_MSG_CHT_STR="[檢查中]_STEP02-%M1_TAG_HOL_FIL1% 找出 %M1_TODAY% 有無設定明日補休假日-[FM-%M1_SENDIP%]"	
echo. >> %M1_LOG%
echo. >> %M1_LOG%
echo Process...Step 2 : %M1_TAG_HOL_FIL1% 找出 %M1_TODAY% 有無設定明日補休假日,進入Step 2檢查....		>> %M1_LOG%
echo Process...Step 2 : %M1_TAG_HOL_FIL1% 找出 %M1_TODAY% 有無設定明日補休假日,進入Step 2檢查....
REM *************************************************************************************************
set M1_FIND_DATE=%x%%y%%z%
REM :: M1_FIND_DATE=YYYYMMDD	 用於傳送接收 FIND *.dat  內日曆日期
...
REM :: M1_TODAY=YYYYMMDD  	 明日日期
set M1_WORK_YN=30
REM M1_WORK_YN=30  SUN FUNC 找出 FILE(%M1_TAG_HOL_FIL1%)找到 %M1_TODAY% ,明日為補休假日  true  L1_STEP02_FIND 
REM M1_WORK_YN=31  SUN FUNC 找出 FILE(%M1_TAG_HOL_FIL1%)找到 %M1_TODAY% ,明日為補休假日  fail  L1_STEP02_NOFIND
...
set M1_FIND_HOL_DATE=%M1_i%	
set M1_WORK_YN=20
SET M1_MSG_CHT_STR="[檢查完成]_檢查日期( %M1_TODAY% )為補休假日,找到補休假日設定檔_FILE (%M1_TAG_HOL_FIL1%-%M1_FIND_DATE%)-[FM-%M1_SENDIP%]"
echo End........Process...Step 2 :  [檢查完成]_日期( %M1_TODAY% )為補休假日,找到補休假日設定檔_FILE (%M1_TAG_HOL_FIL1%-%M1_FIND_DATE%)	>> %M1_LOG%
GOTO L1_CHECK_STEP_END
...
set M1_WORK_YN=21
echo END.....Process...Step 2 :  %M1_TODAY% 不存在 %M1_TAG_HOL_FIL1% ,進入STEP 3檢查明日是為周幾?	>> %M1_LOG%

GOTO L1_CHECK_STEP03

REM *************************************************************************************************
REM :: END FUNC L1_CHECK_STEP02 	找出     明日是否存在   workday.dat 為補上班工作日
REM *************************************************************************************************
...
REM :: (2) 檢查明日是否為 周六或周日 ,    如有代表明日為勞工(一般)例假日
REM :: 取得明日為週 ?
SET M1_MSG_CHT_STR="[檢查中]_STEP03-找出 %M1_TODAY% 是否為 周六或周日,如有代表明日為勞工(一般)例假日-[FM-%M1_SENDIP%]"  
echo Process...Step 3 : 找出 %M1_TODAY% 是否為 周六或周日,如有代表明日為勞工(一般)例假日	>> %M1_LOG%
echo Process...Step 3 : 找出 %M1_TODAY% 是否為 周六或周日,如有代表明日為勞工(一般)例假日
REM *************************************************************************************************
set M1_FIND_DATE=%x%%y%%z%
REM :: M1_FIND_DATE=YYYYMMDD	 用於傳送接收 FIND *.dat  內日曆日期
REM :: set M1_TODAY=%x%%y%%z%
REM M1_TODAY=YYYYMMDD  	 明日日期
set M1_WORK_YN=31
REM M1_WORK_YN=20  SUN FUNC 檢查明日是否為 周六或周日  true  GOTO L1_CHECK_STEP_END
REM M1_WORK_YN=21  SUN FUNC fail 明日日期上班工作日 goto Step 2
...
IF %M1_WEEK_D% EQU 週六 GOTO L1_STEP03_DAY6
IF %M1_WEEK_D% EQU 週日 GOTO L1_STEP03_DAY7
...
echo. >> %M1_LOG%
echo. >> %M1_LOG%
echo *****New Process 明日為周六(一般)例假日時,改再檢查下週一*****					>> %M1_LOG%
::REM 2021-0125 新增明日為周六(一般)例假日時,改再檢查周1(+3)  [+1=周六 ]  [+2=週日 [+3=週一]
::REM today   & Update M1_TAGFILE day **+1 =tommorrow 
...
REM *************************************************************************************************
REM :: END FUNC L1_CHECK_STEP03	(3) 檢查明日是否為 周六或周日 ,如有代表明日為勞工(一般)例假日,1-5為平日工作日
REM *************************************************************************************************
...
echo 系統日期(M1_SYSTEM_DATE)   %M1_SYSTEM_DATE%
echo 檢查是否工作日期(M1_TODAY)	%M1_TODAY%												>> %M1_LOG%
echo.-----------------------------------------------------------------------	>> %M1_LOG%
echo  檢查是否工作日期(%M1_TODAY%)是否為工作日(M1_WORK_YN)%M1_WORK_YN%					>> %M1_LOG%
echo 10,20,30 代表意義說明如下:													>> %M1_LOG%
...
echo. >> %M1_LOG%
echo. **************************************************	>> %M1_LOG%
echo. 程式名稱 Find_TM_Holiday.BAT							>> %M1_LOG%
