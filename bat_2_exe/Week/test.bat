...
REM !!!####20190123######
SET M1_LOG_HEAD=%M1_LOGDIR%\AUTO_SET_DUTY_LOG_HEAD.txt
     SET M1_LOG=%M1_LOGDIR%\AUTO_SET_DUTY_RUNLOG.txt
REM 將 LOG 寫在前面 ***2019/01/21
SET M1_Swap_TMP=%M1_LOGDIR%\AUTO_SET_DUTY_LOG_TMP.txt
REM 每次離開時把M1_LOG 存至 M1_Swap_TMP
SET M1_Swap_OK=%M1_LOGDIR%\AUTO_SET_DUTY_LOG_OK.txt
REM 最後一次離開時把LOG寫在前面, 後面再加上原 M1_LOG


%M1_RUNDRIVE%
CD %M1_RUNDIR% 
msg_str_sec_bye.exe 執行0030_auto_set_duty.bat 5  
...
::timeout /t 36000
echo. **************************************************	>> %M1_LOG_HEAD%
echo. 程式名稱 auto_set_duty.bat						          >> %M1_LOG_HEAD%
...
echo. **************************************************	>> %M1_LOG%
echo. 程式名稱 auto_set_duty.bat						      	 >> %M1_LOG%
...
msg_str_sec_bye.exe 完成0030_auto_set_duty.bat 5 
