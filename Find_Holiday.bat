...
IF NOT EXIST msg_str_sec_bye.eee GOTO L1_RUN_FILE00_ERROR
:L1_END_FILE00_ERROR
IF NOT EXIST %M1_TAG_WRK_FIL1% GOTO L1_RUN_FILE01_ERROR
IF NOT EXIST %M1_TAG_HOL_FIL1% GOTO L1_RUN_FILE02_ERROR
...
:REM now   & date & time::
echo wscript.echo now >%tmp%\tmp99.vbs
for /f "tokens=1 delims=" %%t in ('cscript /nologo %tmp%\tmp99.vbs') do set now=%%t
del %tmp%\tmp99.vbs
::echo. %now%
::timeout /t 36000
::msg_str_sec_bye.exe 執行Find_Holiday.bat 5 
MSG * /TIME:5 "                         執行Find_Holiday.bat        "
echo. **************************************************	>> %M1_LOG_HEAD%
echo. 程式名稱 Find_Holiday.BAT							     	 >> %M1_LOG_HEAD%
echo. 程式執行時間 %now%		 				            	 >> %M1_LOG_HEAD%
echo. **************************************************	>> %M1_LOG_HEAD%
::timeout /t 3600
...
::ECHO %M1_MSG_CHT_STR%
::timeout /t 3600
::msg_str_sec_bye.exe 完成Find_Holiday.bat 5 
MSG * /TIME:5 "                         完成Find_Holiday.bat        "
SET M1_MSG_ICON=64
SET M1_event=ok_msg
GOTO L1_SENDMSG1
...
:L1_RUN_FILE00_ERROR
echo 異常發生找不到 D:\msg\use\msg_str_sec_bye.eee
TIMEOUT /T 30
GOTO L1_END_FILE00_ERROR

:L1_RUN_FILE01_ERROR
msg_str_sec_bye.eee 異常發生找不到_%M1_TAG_WRK_FIL1% 99999
GOTO L1_EXIT_OUT
